...
import os.path
from .mylibw import *
from .dataParse import parseTransformation
from itertools import product
from itertools import combinations_with_replacement
from .PredicateLibV5 import *
tf.compat.v1.disable_eager_execution()

class NLILPRLEngine(object):
    def __init__(self, args, predColl, bgs, disp_fn=None, phase=''):
        ...
        self.plogent = tf.placeholder("float32", [], name='plogent'+self.phase)
        ...
        self.ruleDefintionStringList = [] #used to store string defintion outputs for each rule.
        ...
        for p in self.predColl.preds:
            ...
            self.X0[p.name] = tf.placeholder("float32", [self.args.BS, self.predColl[p.name].pairs_len], name='input_x_'+ self.phase + p.name) #(40,1)
            if p.pFunc is None:
                continue
            #for each class, tensor of size batchsize(40)~based on the background fact size
            self.target_data[p.name] = tf.placeholder("float32", [self.args.BS, self.predColl[p.name].pairs_len] , name='target_data_' + self.phase + p.name) #(40,1)
            self.target_mask[p.name] = tf.placeholder("float32", [self.args.BS, self.predColl[p.name].pairs_len] , name='target_mask_' + self.phase  + p.name) #(40,1)

            #tensors for the weight differences, attempt to code below
            self.target_difference[p.name] = tf.placeholder("float32", [self.args.BS, self.predColl[p.name].pairs_len] , name='target_mask_' + self.phase + p.name) #(40,1)
        ...
        for v in self.predColl.cnts:
            self.thresholds_gt[v.name] = tf.get_variable( 'th_gt_' + self.phase + v.name , shape=(v.no_gt), 
                initializer=tf.constant_initializer(v.gt_init, dtype=tf.float32) ,collections=[tf.GraphKeys.GLOBAL_VARIABLES, 'CONTINUOUS_' + self.phase])
            self.thresholds_lt[v.name] = tf.get_variable( 'th_lt_' + self.phase + v.name , shape=(v.no_lt), 
                initializer=tf.constant_initializer(v.lt_init, dtype=tf.float32) ,collections=[ tf.GraphKeys.GLOBAL_VARIABLES,'CONTINUOUS_'+ self.phase])
            #arg BS (Batch Size) determines the input size, key variable (6)
            self.continuous_inputs[v.name] = tf.placeholder("float32", [self.args.BS,v.dim,self.args.T] ,name='continous_input_' + self.phase +v.name)
    ...
    def define_model(self):
        ...
        for t in range(self.args.T): #Tmax is only 1 here...
            ...
            for p in self.predColl.preds:
                ...
                with tf.variable_scope( "ILP_" + self.phase, reuse=tf.AUTO_REUSE, auxiliary_name_scope=False):
    ...
    def train_model(self):
        ...
        self.LR = tf.placeholder("float", shape=(),name='learningRateOPS')
        ...
        self.optimizer = tf.train.AdamOptimizer(learning_rate=self.LR, beta1=self.args.BETA1,
                       ...
                       use_locking=False, name='Adam'+self.phase)
                            ...
                            if p.pFunc is not None:
                                ...
                                if s is not None:
                                    if len(s)>0:
                                        ...
                                        self.ruleDefintionStringList.append(p.name+ '(' + ','.join(variable_list[0:p.arity]) + ')  \n'+s)
